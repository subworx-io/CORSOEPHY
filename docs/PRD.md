...
> - Offene Entscheidung **#6 (Stadt-Story-Größe) auf ENTSCHIEDEN gesetzt** — die Entscheidung fiel am 15. Juli, das PRD hing hinterher. §4.6 entsprechend präzisiert: max. 8 Momente, kein Mindest-Schwellwert, kein Fake-Auffüllen.
> - Offene Entscheidung **#7 (Privater Korso)** mit dem Hinweis versehen, dass sie mit dem Push-Feature in Roadmap-Phase 1 fällig wird.
>
...
- **Clip-Lebensdauer: genau 24 h ab dem Post.** Jeder Moment trägt seine eigene Uhr (`posts.expires_at = created_at + 24 h`), es gibt keinen stadtweiten Reset mehr. Danach ist er überall weg — auch für den Autor.
- **Genau ein lebender Moment pro Person.** Ein neuer Post beendet den vorherigen sofort.
...
- Gleichzeitig mit der Stadt-Story-Ziehung: die Stadt sieht den Corso des vergangenen Zyklus und bekommt im selben Moment die neue Aufgabe.
...
- Fixe Uhrzeit: **21:00 Uhr**. Ganze Stadt sieht dieselben Momente, **maximal 8**. Die Auswahl steht eingefroren bis zur nächsten Ziehung 24 h später.
  - **Präzisiert (15. Juli, Entscheidung #6):** Die Story läuft mit so vielen einwilligenden Clips, wie an dem Tag da sind — **kein Mindest-Schwellwert, kein Fake-Auffüllen.** Bei dünner Nutzerbasis ist eine Story aus 3 echten Clips richtig, eine aus 8 aufgefüllten falsch.
...
- **Ein gezogener Moment bleibt die ganze Story lang sichtbar**, auch wenn seine 24 h währenddessen ablaufen — in Discovery/Ich folge/Rücklauf ist er dann weg, im Stadt Corso bleibt er stehen. Das Rampenlicht verlängert den Moment; ein Clip kann so bis zu ~48 h im Corso stehen.
...
5. Stadt-Story-Screen (21:00, vertikaler Karten-Feed wie Discovery, 8 Momente)
...
### Kern-Flows
- **A — Erster Abend:** Onboarding → ID → Heute → Push 21:00 → Stadt-Story → Discovery → folgen → optional Post.
...
| 6 | Stadt-Story-Frequenz/-Größe Tuning | **ENTSCHIEDEN (15. Juli)** ✓ — so viele einwilligende Clips wie da sind, max. 8, kein Minimum, kein Fake-Auffüllen (§4.6) |
| 7 | Privater Korso (Push 19–22 Uhr) — genaue Mechanik | offen — **wird mit dem Push-Feature in Roadmap-Phase 1 fällig** |
